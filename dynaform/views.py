...
from .graphs import barchart, horizontal_barchart, piechart
...
class Results(TemplateView):
    ...
    def get_context_data(self, **context):
        vegetarian_options = {
            k: v
            for k, v in Preference.objects.values("vegetarian")
            .annotate(Count("vegetarian"))
        ...
        participative_kitchen = {
            k: v
            for k, v in Preference.objects.values("participative_kitchen")
        ...
        context["participative_kitchen"] = barchart(
            data=[
                participative_kitchen.get(k, 0) for k, _ in Preference.ETHIC_CHOICES_F
            ],
        ...
        for value in ["organic", "sustainable", "fairtrade", "local", "zero_waste"]:
            toto = {
                k: v
                for k, v in Preference.objects.values(value)
        ...
        entity_type = {
            k: v
            for k, v in Preference.objects.values("entity_type")
        ...
        price_markup = {
            k: v
            for k, v in Preference.objects.values("price_markup")
